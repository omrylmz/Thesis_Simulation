...
data class CentralizedPDDLPlan(val time: Int, val plan: MutableMap<String, MutableList<String>>)

class CentralizedBehaviour : LTDBehaviourHttp() {
    ...
    override fun communicate(agent: AgentImp) {
        if (timeStep == 0) {
            ...
            if (agent.getID() == 1) {
                ...
                val respBody = super.httpPOSTResponse(stateJson, "http://localhost:8888/")
                println(respBody)
    ...
    private fun loadPlanJSON(agent: AgentImp, respBody: String) {
        ...
        singlePlan.forEach {
            if (it.startsWith("move ")) {
                ...
                // if (0 < agent.getID()) {println("move>>>> " + matchLst[0].groups[1]?.value)}
                // val xy = matchLst[0].groups[1]?.value?.slice(1..(matchLst[0].groups[1]?.value?.length as Int -1))//?.split(" ")?.toList()
