...
data class CentralizedByPDDLPDDLPlan(val time: Int, val plan: MutableMap<String, MutableList<String>>)

class CentralizedByPDDLBehaviour : LTDBehaviourHttp() {
    ...
    override fun communicate(agent: AgentImp) {
        if (timeStep == 0) {
            ...
            if (agent.getID() == 1) {
                ...
                val respBody = super.httpPOSTResponse(stateJson, "http://localhost:8888/pddlmas")
                println(respBody)
                loadPlanJSON(agent, respBody)
                println(respBody)

                if (multiAgentPlan.toString().length > 10) {
                    agentReps.forEach {
                        if (it.getID() != agent.getID()) {
                            agent.sendMessage(it, respBody)
                        }
                    }
                }
            }
            ...
            if (agent.getID() != 1) {
                val msgs = agent.getMessages().toList()
                // msgs.forEach {println("Message: " + it.getFrom() + " -> " + it.getTo() + ": " + it.getMessage())}
                msgs.getOrNull(0)?.getMessage()?.let { loadPlanJSON(agent, it) }
                agent.clearMessages()
            }
            simpleSinglePlan.forEach(::println)
        } else {
    ...
    fun getDynamic(agent: AgentImp) {
        ...
        val color = agent.getEnvironment().getAgentWorld().getAgent(agent.getID()).getColor().get().toString()
        agentsData.put(
            Pair<Int, Int>(agent.getX(), agent.getY()), if (color.equals("")) {
